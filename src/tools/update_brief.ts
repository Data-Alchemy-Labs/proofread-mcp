...
export const updateBrief = defineTool({
  ...
  description:
    ...
    "Passing only title renames the brief without a check. A changed text counts as one check. " + LIMITS_NOTE + " " + BRIEF_NOTE,
  inputSchema: {
  ...
  async run({ id, text, title }, { client }, extra) {
    if (text === undefined && title === undefined) return fail("give text (the whole edited brief), title (a new name), or both.");
    if (!client.hasApiKey()) return fail(BRIEF_NEEDS_KEY);
    try {
      const updated = await client.updateBrief(id, { ...(text !== undefined ? { text } : {}), ...(title !== undefined ? { title } : {}) }, extra.signal);
      ...
      return ok(formatUpdatedBrief(updated, reportId, text !== undefined), {
        id: updated.id,
