...
export const renderReport = defineTool({
  ...
  async run({ report_id, report }, { client }, extra) {
    ...
    if (report_id) {
      ...
      if (!source) return fail(new Error(`no report ${report_id} in memory (this server keeps the last 50 checks until it exits). Run the check again and use the new id.`));
